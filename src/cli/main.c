...
int main(int argc, char* argv[]) {
  ...
  if (argc == 3) {
    if (strcmp(argv[1], "build") == 0) {
      ...
      VosDelegate delegate = (VosDelegate){.alloc = malloc,
                                           .realloc = realloc,
                                           .free = free,
                                           .error_handler = err,
                                           .printf = printf};
      ...
      double stop_timestamp =
          (double)((stop.tv_sec * 1000) + (stop.tv_usec / 1000));
      fprintf(stderr, "\x1b[1mSuccessfully compiled in %.3fs\x1b[0m",
              (stop_timestamp - start_timestamp) / 1000.0);
    }
  } else {
    fprintf(stderr,
            "Usage: vos build <file.vos>\n       vos run <file.vos>\n       "
            "vos exec <file.vbc>\n");
    return 0;
  }
  return 0;
