...
typedef enum {
  ...
  LIST_STATEMENT, /* Same as COMPOUND_STATMENT, but has a static type. Used for
                     arrays. */
  ...
  BINARY_EXPR,   /* Operation with two values. Ex: addition, subtract,
                    multiplication, etc. */
...
typedef struct {
  ASTNodeType type;
  int size;
  void *declaration; /* Declaration of this node's object if available. Not
                        filled out until phase 2. */
} ASTNode;
