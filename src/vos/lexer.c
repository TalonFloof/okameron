...
utf8_int32_t lexer_peekNextChar(Lexer* lex) {
  if ((*lex->pos) == '\0')
    return '\0'; /* To keep us from looking past the end of the string */
  utf8_int32_t c;
...
Token lexer_add_token(Lexer* lex, TokenType type) {
  Token tok =
      (Token){.type = type,
              .start = lex->start,
              .length = ((uintptr_t)(lex->pos)) - ((uintptr_t)(lex->start)),
              .lineno = lex->start_lineno,
              .colno = lex->start_colno};
  lex->start = lex->pos;
...
void lexer_scan_comment(Lexer* lex) {
  ...
  while (1) {
    ...
    if (c == '\0') {
      err("%s %i:%i Comment extends beyond EOF", lex->filename, lex->lineno,
          lex->colno);
    }
    if ((c == '*') && (c2 == '/')) {
      lexer_nextChar(lex);
...
Token lexer_scan_identifier(Lexer* lex) {
  ...
  if ((((uintptr_t)(lex->pos)) - ((uintptr_t)(lex->start)) == 3)) {
    if (utf8ncmp(lex->start, "vos", 3) == 0) {
      err("%s %i:%i Identifier \"vos\" is not fluffy enough. Did you mean "
          "\"🦊\"?",
          lex->filename, lex->lineno, lex->colno); /* Haha, da funny mesg */
    }
  }
  TokenType type = lexer_keyword(
      lex->start, ((uintptr_t)(lex->pos)) - ((uintptr_t)(lex->start)));
...
Token lexer_scan_operator(Lexer* lex) {
  ...
  switch (c) {
    ...
    case '|':
      if (c2 == '|') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_BOOL_OR;
      } else
        token = TOKEN_OPERATOR_UNARY_OR;
      break;
    case '&':
      if (c2 == '&') {
        lexer_nextChar(lex);
    ...
    case '!':
      if (c2 == '=') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_NOTEQUAL;
      } else
        token = TOKEN_OPERATOR_BOOL_NOT;
      break;
    case '<':
      if (c2 == '=') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_LESSEQUAL;
      } else if (c2 == '<') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_LSHIFT;
      } else
    ...
    case '>':
      if (c2 == '=') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_MOREEQUAL;
      } else if (c2 == '>') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_RSHIFT;
      } else
    ...
    case ':':
      if (c2 == '=') {
        lexer_nextChar(lex);
        token = TOKEN_OPERATOR_IMPLY_ASSIGN;
      } else
        token = TOKEN_OPERATOR_COLON;
      break;
    case ',':
    ...
    case '}':
      ...
      break;
    default: {
      err("%s %i:%i Unrecognized Operator", lex->filename, lex->lineno,
          lex->colno);
    }
  }
...
Token lexer_scan_string(Lexer* lex) {
  ...
  while (1) {
    if (c == '\0') {
      err("%s %i:%i EOF while lexing String literal", lex->filename,
          lex->lineno, lex->colno);
    }
    if (c == '\n') {
      err("%s %i:%i EOL while lexing String literal", lex->filename,
          lex->lineno, lex->colno);
    }
    if (c == '\\') {
      lexer_nextChar(lex);
      if (lexer_peekChar(lex) == '\0') {
        err("%s %i:%i EOF while lexing String literal", lex->filename,
            lex->lineno, lex->colno);
      }
    }
...
loop:
  ...
  if ((c == '/') && (lexer_peekNextChar(lex) == '*')) {
    lexer_scan_comment(lex);
    goto loop;
  }
  if ((c == '_') || isalpha(c) || (c >= 0xc0)) {
    return lexer_scan_identifier(lex);
  }
  if (is_operator(c)) {
    return lexer_scan_operator(lex);
  }
  if (isdigit(c) || (c == '-' && isdigit(lexer_peekNextChar(lex)))) {
