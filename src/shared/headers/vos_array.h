...
#define ARRAY_DEFAULT_SIZE                                                \
  8 /* Decreasing this number will reduce performance, but improve memory \
       usage. Increasing this number increases memory usage, but improves \
       performance. */
...
#define array_decl_init(_t, _v) \
  _t _v;                        \
  array_init(_v)
#define array_destroy(d, v) \
  if ((v).p) d->free((v).p)
...
#define array_resize(d, type, v, n) \
  (v).m += n;                       \
  (v).p = (type *)d->realloc((v).p, sizeof(type) * (v).m)
#define array_resize0(d, type, v, n)                                    \
  (v).p = (type *)d->realloc((v).p, sizeof(type) * ((v).m + n));        \
  (v).m ? vos_memset((v).p + (sizeof(type) * n), 0, (sizeof(type) * n)) \
        : vos_memset((v).p, 0, (sizeof(type) * n));                     \
  (v).m += n
