...
#define array_destroy(v)           if ((v).p) free((v).p)
...
#define array_push(type, v, x)     {if ((v).n == (v).m) {                                        \
                                    (v).m = (v).m? (v).m<<1 : ARRAY_DEFAULT_SIZE;                \
                                    (v).p = (type*)realloc((v).p, sizeof(type) * (v).m);}        \
                                    (v).p[(v).n++] = (x);}
#define array_resize(type, v, n)   (v).m += n; (v).p = (type*)realloc((v).p, sizeof(type) * (v).m)
#define array_resize0(type, v, n)  (v).p = (type*)realloc((v).p, sizeof(type) * ((v).m+n));    \
